...
func (c *Client) readPump() {
	...
	for {
		...
		case "nodes":
			message = []byte(fmt.Sprintf("%v > %v\n", timestamp, galanodes.GetNodeInfo()))
			break
		...
		case "reboot":
			if len(parts) == 1 {
				message = []byte(fmt.Sprintf("%v > %v\n", timestamp, "Please pass a node index or addres, all with reboot command."))
			} else {
		...
		case "find":
			if len(parts) == 1 {
				message = []byte(fmt.Sprintf("%v > %v\n", timestamp, "Please pass a string with find command"))
			} else {
				message = []byte(fmt.Sprintf("%v > %v\n", timestamp, galanodes.FindNodeInfo(parts[1])))
			}
			break
